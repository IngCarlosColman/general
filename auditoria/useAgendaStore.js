...
export const useAgendaStore = defineStore('agenda', () => {
  ...
  const addContactToAgenda = async (contact_cedula) => {
    try {
      const response = await axiosClient.post('/private-agenda/agenda', { contact_cedula }); 
      
      // Actualizar el estado con la nueva cédula
      privateAgendaCedulas.value.push(contact_cedula);
      
      // Se vuelve a cargar la agenda para obtener los datos completos del nuevo contacto
      await fetchAgendaContacts();
  ...
  const removeContactFromAgenda = async (cedula) => {
    try {
      ...
      privateAgendaCedulas.value = privateAgendaCedulas.value.filter(c => c.contact_cedula !== cedula);
  ...
  const searchPublicContacts = async (query) => {
    ...
    try {
      const response = await axiosClient.get(`/general`, {
        params: { search: query }
      });
