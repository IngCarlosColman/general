...
export const useCatastroStore = defineStore('catastro', () => {
  ...
  const searchCadastre = async (item, tipoPropiedad, isRural) => {
    ...
    try {
      // PASO 1: Llamada a la API de Catastro para obtener los propietarios
      ...
      // PASO 2: Llamada a la API de tu back-end para guardar los propietarios
      ...
      if (batchResult.success) {
        ...
        let telefonosPropietario = ''; // Variable para guardar los teléfonos
        ...
        if (propietarios.length > 0) {
          ...
          // --- LÓGICA AGREGADA PARA OBTENER TELÉFONOS ---
          const cedulas = propietarios
                           .map(p => p.propNroDocumento)
                           .filter(Boolean)
                           .join(',');

          if (cedulas) {
            // Realizar la llamada al nuevo endpoint del back-end
            const responseTelefonos = await axiosClient.get(`/telefonos?cedulas=${cedulas}`);
            
            // Procesar la respuesta para obtener una lista única de números
        ...
        // PASO 3: Actualizar el item del front-end con toda la información enriquecida
        const updatedItem = {
          ...item,
          propietario_completo: nombrePropietario,
          cedula_propietario: cedulaPropietario,
          telefono: telefonosPropietario, // Aquí se añade el campo "telefono"
        };
