...
const removeContact = async (cedula) => {
  // Verificación de seguridad para evitar la llamada a la API si 'cedula' no es válido
  if (!cedula) {
    console.error('La cédula para eliminar el contacto es indefinida.');
    showSnackbar('Error: No se pudo identificar el contacto a eliminar.', 'error');
    return;
  }
